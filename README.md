...
|--------|----------|-------------|
| Sensirion SGP41 | VOC index / NOx index | 0x59 |
| Sensirion STCC4 | CO2 | 0x64 (0x65 if ADDR is high) |
| Sensirion SFA40 | formaldehyde (HCHO) | 0x5D |
| Sensirion SHT40 | temperature / humidity | none — connected to the STCC4's private I2C bus and read through the STCC4 |
| Bosch BMV080 | PM1 / PM2.5 / PM10 | 0x54 |

Boards up to `hw/air-stcc4/0.3.0` carry an SGP40 (VOC only) instead of the
SGP41 and have no SFA40. The two SGP parts share address 0x59; the test
firmwares read the SGP4x feature set at startup to tell them apart, so one
build runs on either revision.

This is a 3.3 V board with no voltage regulator — don't use it with 5 V
circuits. To simplify it, the board is opinionated about I2C
addresses and doesn't expose address selection pins for chips that
have them. For reliability I don't recommend putting too many chips on
one I2C bus.

![front](images/front-0.4.0.jpg) ![back](images/back-0.4.0.jpg)

## AI Disclosure

The board was designed by a human. CI and firmware were generated by AI.
...
description (Bill of Form, Fit and Features) - it's a tldr; for
important facts about the board. The [BOFFF workflow](.github/workflows/bofff.yml) does nothing but
...
| Directory | Platform | SGP41 | STCC4 | SFA40 | SHT40 (via STCC4) | BMV080 |
|-----------|----------|-------|-------|-------|-------------------|--------|
| [firmware/circuitpython/](firmware/circuitpython/) | CircuitPython 9+ | yes | yes | yes³ | yes | presence check only¹ |
| [firmware/arduino/](firmware/arduino/) | Arduino / ESP32 | yes | yes | yes | yes | yes |
| [firmware/esphome/](firmware/esphome/) | ESPHome / ESP32 | yes | yes² | yes⁴ | yes² | yes |
...
³ Via [romkey/circuitpython-sfa40](https://github.com/romkey/circuitpython-sfa40).

⁴ ESPHome's `sfa40` component landed upstream after 2026.8, so the ESPHome
build uses the beta channel until a stable release includes it.
